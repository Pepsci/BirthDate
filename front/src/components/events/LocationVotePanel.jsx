...
const LocationVotePanel = ({ shortId, options, myVote = null, isOrganizer, invitations = [], onVoteChange }) => {
  const handleVote = async (locationId) => {
    try {
      await apiHandler.post(`/events/${shortId}/vote/location`, { locationId });
  ...
  return (
    <div className="location-vote-panel" style={{ background: "var(--bg-secondary)", padding: "20px", borderRadius: "15px" }}>
      <h3 style={{ margin: "0 0 15px 0" }}>📍 Vote pour le lieu</h3>
      <p style={{ fontSize: "0.9rem", color: "var(--text-secondary)", marginBottom: "20px" }}>
        Choisissez le lieu qui vous convient le mieux.
      </p>
      
      <div className="location-options" style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
        {options.map((opt) => {
            ...
            >
              ...
              >
                <div style={{ display: "flex", flexDirection: "column" }}>
                  ...
                  <span style={{ fontSize: "0.85rem", opacity: 0.8 }}>{opt.address}</span>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  {/* Nombre de votants */}
                  <span style={{ fontSize: "0.85rem", fontWeight: "bold", opacity: 0.7 }}>
                    {invitations.filter(inv => inv.locationVote === opt._id).length} votant(s)
                  </span>
                  {!isOrganizer && (
                    <div style={{
