...
const GiftProposalPanel = ({ shortId, isOrganizer }) => {
  ...
  const [editingGift, setEditingGift] = useState(null); // { _id, name, url, price }
                ...
                {isEditing ? (
                  <form onSubmit={handleEditGift} style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                    <input
                      ...
                      onChange={(e) => setEditingGift(prev => ({ ...prev, name: e.target.value }))}
                      required
                      style={{ padding: "7px", borderRadius: "5px", border: "1px solid var(--border-color)", background: "var(--bg-secondary)", color: "var(--text-primary)" }}
                    />
                    <input
                      type="text"
                      placeholder="Lien (URL)"
                      value={editingGift.url}
                      onChange={(e) => setEditingGift(prev => ({ ...prev, url: e.target.value }))}
                      style={{ padding: "7px", borderRadius: "5px", border: "1px solid var(--border-color)", background: "var(--bg-secondary)", color: "var(--text-primary)" }}
                    />
                    <input
                      type="number"
