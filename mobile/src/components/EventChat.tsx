...
/**
 * Chat d'un événement (chiffré de bout en bout).
 *
 * Deux usages :
 * - `app/event/chat/[shortId].tsx` : écran plein ;
 * - `app/event/[shortId].tsx` en paysage sur grand écran (`embedded`) :
 *   panneau de droite, l'événement restant affiché à gauche. L'en-tête de la
 *   pile appartient alors à l'ÉVÉNEMENT : le bouton silencieux passe dans une
 *   barre interne, et le lien « ouvrir l'événement » disparaît (il est déjà
 *   à gauche).
 */
export default function EventChat({
  ...
  if (loading) {
    return (
      <View style={styles.center}>
        {!embedded && <Stack.Screen options={{ title: "Chat" }} />}
  ...
  const body = (
    ...
    >
      ...
      {/* En panneau, l'événement est déjà affiché à gauche. */}
      {!embedded && (
        <>
...
const makeStyles = (c: ThemeColors) =>
  StyleSheet.create({
    ...
    embeddedRoot: { flex: 1 },
    // Barre titre + silencieux quand le chat est un panneau (grand écran)
    embeddedHeader: {
      flexDirection: "row",
      alignItems: "center",
