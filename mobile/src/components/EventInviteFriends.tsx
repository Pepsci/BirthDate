 ...
 *
 * `onDone` est appelé après l'envoi ou sur « Plus tard ». Par défaut (écran
 * plein) on retourne à l'événement ; en panneau, l'appelant recharge
 * l'événement et rend la main au chat.
 */
export default function EventInviteFriends({
  shortId,
  embedded = false,
  onDone,
}: {
  ...
  const done = () =>
    onDone ? onDone() : router.replace(`/event/${shortId}`);
...
const makeStyles = (c: ThemeColors) =>
  StyleSheet.create({
    ...
    list: { padding: 12, gap: 8, paddingBottom: 24, ...readingPane },
    sectionTitle: {
      fontSize: 13,
      fontWeight: "700",
      color: c.sub,
