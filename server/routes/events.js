...
router.get("/mine", isAuthenticated, async (req, res) => {
  try {
    ...
    const organizedEvents = await Event.find({ organizer: userId }).populate("forPerson", "name surname");
...
router.get("/:shortId", async (req, res) => {
  try {
    ...
    if (req.headers.authorization && req.headers.authorization.split(" ")[0] === "Bearer") {
      ...
      try {
        const payload = require("jsonwebtoken").verify(token, process.env.TOKEN_SECRET);
        userId = payload._id;
      } catch (err) {}
    } else if (req.cookies && req.cookies.authToken) {
      try {
        const payload = require("jsonwebtoken").verify(req.cookies.authToken, process.env.TOKEN_SECRET);
        userId = payload._id;
      } catch (err) {}
    }
    ...
    if (hasFullAccess) {
      return res.status(200).json({ ...event.toObject(), hasFullAccess, myRsvpStatus });
    } else {
...
router.post("/:shortId/invite", isAuthenticated, async (req, res) => {
  try {
    ...
    const baseUrl = process.env.NODE_ENV === "production" ? "https://birthreminder.com" : "http://localhost:5173";
    ...
    if (userIds && userIds.length > 0) {
      for (const uid of userIds) {
        ...
        if (!existing) {
          ...
          if (targetedUser) {
            await sendEventInvitationEmail(targetedUser.email, event, event.organizer.name, eventUrl);
          }
        }
      }
    }
...
router.post("/:shortId/join", async (req, res) => {
  try {
    ...
    if (event.maxGuests !== null) {
      const currentGuests = await EventInvitation.countDocuments({ event: event._id, status: { $in: ["accepted", "maybe"] } });
    ...
    if (tokenPayload) {
      ...
      if (!guestName) {
        return res.status(200).json({ message: "Code valide, accès accordé", unlockSession: true });
      }

      invitation = await EventInvitation.findOne({ event: event._id, externalEmail, guestName });
      if (!invitation) {
        invitation = await EventInvitation.create({ event: event._id, externalEmail, guestName, joinedViaCode: true, status: "accepted" });
      }
    }
 ...
 */
router.put("/:shortId/rsvp", isAuthenticated, checkEventAccess, async (req, res) => {
  try {
    const { status } = req.body;
    // req.event et req.invitation sont fournis par le middleware
    if (req.userRole === "organizer") {
      return res.status(400).json({ message: "L'organisateur ne peut pas modifier son RSVP via cette route." });
    }
 ...
 */
router.post("/:shortId/vote/date", isAuthenticated, checkEventAccess, async (req, res) => {
  try {
    const { dates } = req.body; 
    
    if (req.userRole === "organizer") {
      return res.status(400).json({ message: "L'organisateur ne vote pas via cette route." });
    }
 ...
 */
router.post("/:shortId/vote/location", isAuthenticated, checkEventAccess, async (req, res) => {
  try {
 ...
 */
router.post("/:shortId/gifts", isAuthenticated, checkEventAccess, async (req, res) => {
  try {
 ...
 */
router.get("/:shortId/gifts", isAuthenticated, checkEventAccess, async (req, res) => {
  try {
    const event = req.event;

    const proposals = await EventGiftProposal.find({ event: event._id }).populate("proposedBy", "name surname");
 ...
 */
router.post("/:shortId/gifts/:giftId/vote", isAuthenticated, checkEventAccess, async (req, res) => {
  try {
    ...
    if (hasVoted) {
      proposal.votes = proposal.votes.filter(id => id.toString() !== userId);
    } else {
